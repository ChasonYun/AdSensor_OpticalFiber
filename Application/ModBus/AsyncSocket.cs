...
namespace OpticalFiber
{
    ...
    {
        public StateObject(int recLength)
        {
            recBuffer = new byte[recLength];
        }
    ...
    public class AsyncSocket
    {
        ...
        Socket clientSocket;
        int deviceNo;
        public ManualResetEvent connecDone = new ManualResetEvent(false);
        public ManualResetEvent sendDone = new ManualResetEvent(false);
        public ManualResetEvent receiveDone = new ManualResetEvent(false);


        public AsyncSocket(IPEndPoint iPEndPoint)
        {
            try
            {
                if (iPEndPoint == null)
                ...
                clientSocket.BeginConnect(iPEndPoint, new AsyncCallback(ConnectCallback), clientSocket);//连接
                connecDone.WaitOne(500);
        ...
        private void WorkThread()
        {
            try
            {
                ...
                while (!DataClass.cancellationTokenSource.IsCancellationRequested)
                {
                    ...
                    Send(clientSocket, IntsToBytes(100, 1, 1, 48));//发送
                    sendDone.WaitOne();
                    //Send(100, 1, 1, 48);
                    tempBytes.AddRange(Recevice(clientSocket, (9 + 48 / 8)));
                    receiveDone.WaitOne();
                    //tempBytes.AddRange(tcp.Receive(9 + 48 / 8));
                    ...
                    tempBytes.Clear();
                    tempBytes.AddRange(tempbyte);
                    Send(clientSocket, IntsToBytes(100, 3, 1, 35));//发送
                    sendDone.WaitOne();
                    tempBytes.AddRange(Recevice(clientSocket, (9 + 35 * 2)));
                    receiveDone.WaitOne();

                    //Send(100, 3, 1, 35);
                    //tempBytes.AddRange(tcp.Receive(9 + 35 * 2));
                    ...
                    for (int i = 1; i <= DataClass.list_DeviceParam[deviceNo].struct_deivceParam.channelNum; i++)
                    {
                        ...
                        Send(clientSocket, IntsToBytes(i, 3, 1, 14));//发送
                        sendDone.WaitOne();
                        tempBytes.AddRange(Recevice(clientSocket, (9 + 14 * 2)));
                        receiveDone.WaitOne();

                        //Send(i, 3, 1, 14);
                        //tempBytes.AddRange(tcp.Receive(9 + 14 * 2));
                    ...
                    for (int i = 1; i < DataClass.list_DeviceParam[deviceNo].struct_deivceParam.channelNum; i++)
                    {
                        ...
                        for (int j = 1; j <= count; j++)
                        {
                            ...
                            Send(clientSocket, IntsToBytes(i, 4, (j - 1) * 125 + 1, 125));//发送
                            sendDone.WaitOne();
                            tempBytes.AddRange(Recevice(clientSocket, (9 + 125 * 2)));
                            receiveDone.WaitOne();

                            //Send(i, 4, (j - 1) * 125 + 1, 125);
                            //tempBytes.AddRange(Receive(9 + 125 * 2));
                            ...
                            Send(clientSocket, IntsToBytes(i, 4, 10001 + (j - 1) * 125, 125));//发送
                            sendDone.WaitOne();
                            tempBytes.AddRange(Recevice(clientSocket, (9 + 125 * 2)));
                            receiveDone.WaitOne();
                    ...
                    for (int i = 1; i <= DataClass.list_DeviceParam[deviceNo].struct_deivceParam.channelNum; i++)
                    {
                        for (int j = 1; j <= DataClass.list_DeviceChannelParam[deviceNo].struct_DeviceChannelParam.struct_ChannelParams[i].partition; j++)
                        {
                            ...
                            Send(clientSocket, IntsToBytes(i + 200, 3, 100 * j + 1, 44));//发送
                            sendDone.WaitOne();
                            tempBytes.AddRange(Recevice(clientSocket, (9 + 44 * 2)));
                            receiveDone.WaitOne();

                            //Send(i + 200, 3, 100 * j + 1, 44);
                            //tempBytes.AddRange(tcp.Receive(9 + 44 * 2));
        ...
        private byte[] Recevice(Socket client, int recLength)
        {
            byte[] recbuffer = null;
            try
            {
                StateObject stateObject = new StateObject(recLength);
                stateObject.socketClient = client;
                client.BeginReceive(stateObject.recBuffer, 0, recLength, SocketFlags.None, new AsyncCallback(ReceiveCallback), stateObject);
                recbuffer = stateObject.recBuffer;
            }
            catch (Exception e)
            {
                DataClass.ShowErrMsg(e.Message);
        ...
        private void ReceiveCallback(IAsyncResult asyncResult)
        {
            try
            {
                ...
                Socket socket = stateObject.socketClient;
                int byteRead = socket.EndReceive(asyncResult);
                receiveDone.Set();
            }
            catch (Exception e)
            {
